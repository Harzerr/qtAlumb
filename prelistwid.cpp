...
//    auto src_height = src_pixmap.height();
    ...
    pItem->setSizeHint(QSize(PREITEM_SIZE, PREICON_SIZE));
    this->addItem(pItem);
...
void PreListwid::SlotUpdatePreList(QTreeWidgetItem *tree_item)
{
    if(!tree_item){
        return ;
    }
...
void PreListwid::SlotUpdateSlect(QTreeWidgetItem *tree_item)
{
    if(!tree_item){
        return ;
    }
