    ...
    : QWidget{parent},_factor(0.0), _cur_item(nullptr), _b_start(false)
{
    ...
    connect(_timer, &QTimer::timeout, this, &PicAnimationWid::TimeOunt);
}
...
void PicAnimationWid::SlidePre()
{
    ...
    auto* cur_pro_item = dynamic_cast<ProTreeItem*>(_cur_item);
    auto* previous_item = cur_pro_item->GetPreItem();
    if(!previous_item){
          return ;
    }
    SetPixmap(previous_item);
    update();
}
...
void PicAnimationWid::SlotUpdateSelectShow(QString path)
{
...
void PicAnimationWid::SlotUpdateStartOrStop()
{
    if(!_b_start){
          _factor = 0;
          _timer->start(25);
